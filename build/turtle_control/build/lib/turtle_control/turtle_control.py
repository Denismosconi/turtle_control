...
class ControlTurtle(Node):
    def __init__(self, target_x, target_y, Kp_angular, Kp_linear, experiment_name=None):
        ...
        self.max_simulation_time = 30.0  # segundos
        self.convergence_timeout = 5.0   # segundos para convergência
        self.stagnation_counter = 0
        self.max_stagnation_cycles = 100
        self.min_linear_speed_threshold = 0.001
        self.min_angular_speed_threshold = 0.001
    ...
    def callback_controle(self, posicao: Pose):
        ...
        if self.estado == "INICIAR":
            ...
            if abs(self.error_x) > self.linear_tolerance or abs(self.error_y) > self.linear_tolerance:
                ...
                # Verificar se velocidade está abaixo do limite mínimo
                if (abs(self.vetor_velocidade.linear.x) < self.min_linear_speed_threshold and
                    abs(self.vetor_velocidade.angular.z) < self.min_angular_speed_threshold):
                    
                    self.stagnation_counter += 1
                    if self.stagnation_counter > self.max_stagnation_cycles:
                        # Considera convergido mesmo sem atingir tolerância
                        self.estado = "FINALIZADO"
                else:
                    self.stagnation_counter = 0

                # Verificar timeout por tempo total
                if elapsed_time > self.max_simulation_time:
                    self.get_logger().warn("Timeout alcançado - Finalizando simulação")
                    self.estado = "FINALIZADO"

                # Verificar timeout para convergência
                if (abs(self.error_x) < 0.01 and abs(self.error_y) < 0.01):
                    if self.convergence_start_time is None:
                        self.convergence_start_time = elapsed_time
                    elif (elapsed_time - self.convergence_start_time) > self.convergence_timeout:
                        # Está próximo o suficiente por tempo suficiente
                        self.estado = "FINALIZADO"
                else:
                    self.convergence_start_time = None
        ...
        # Inicializar erro_angle com 0 se não estiver definido
        if not hasattr(self, 'erro_angle'):
            self.erro_angle = 0.0
        ...
        # Inicializar velocidade_x e velocidade_y se não estiverem definidas
        if not hasattr(self, 'velocidade_x'):
            self.velocidade_x = 0.0
        if not hasattr(self, 'velocidade_y'):
...
###########################################################################################
    ...
    def save_to_csv(self):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            # Cabeçalho CORRIGIDO
            writer.writerow([
                'time', 'pos_x', 'pos_y', 'pos_theta', 'target_angle_rad', 'error_angle_rad',
                'error_x', 'error_y', 'error_distance', 'vel_linear_x', 'vel_angular_z',
            ...
            for i in range(n_samples):
                writer.writerow([
                    ...
                    self.pos_theta[i] if i < len(self.pos_theta) else 0,
                    self.angle_rad_list[i] if i < len(self.angle_rad_list) else 0,  # Alterado de angle_deg_list
                    ...
                    self.error_distance[i] if i < len(self.error_distance) else 0,
